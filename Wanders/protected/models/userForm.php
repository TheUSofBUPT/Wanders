...
class userForm extends CFormModel
{
	...
	public function rules()
	{
		return array(
			...
			array('username, password, password2, school, email, stunumber, realname, verifyCode', 'required','message'=>'信息未填写完全','on'=>'wikerRegister'),
			...
			array('username, password, email, address, school, stunumber, realname, gender', 'length', 'max'=>40,'message'=>'信息长度不能超过40','on'=>'hirerRegister,wikerRegister'),
    ...
    public function emailAuthenticate()
	{
         ...
         if($this->_authenticity===null)
        {	
        	$this->_authenticity=new EmailIdentity();		
			return $this->_authenticity->emailAuthenticate();
		}
		return false;
	}
	...
	public function hirerRegister()
	{
		...
		$result = Yii::app()->db -> createCommand($sql_command)-> queryAll();		
		if($result)
		{
			$this->addError('username','该用户名已经被注册');
			return false;
		}
		$sql_command = "insert into nb_buyers (`username`,`password`,`email`,`address`) values ( '".$this->username."','".md5($this->password)."','".$this->email."','".$this->address."')";		
		$command = Yii::app()->db -> createCommand($sql_command);	        
		if($command->execute())
		{
			return true;
		}else
	...
	public function wikerRegister()
	{
		...
		$sql_command = "insert into nb_sellers (`username`,`password`,`email`,`gender`,`school`,`stunumber`,`realname`) values ( '".$this->username."','".md5($this->password)."','".$this->email."','".($this->gender == '女'? 0 : 1)."','".$this->school."','".$this->stunumber."','".$this->realname."')";		
		$result = Yii::app()->db-> createCommand($sql_command)->execute();		
		if($result)
		{
			return true;
		}else
